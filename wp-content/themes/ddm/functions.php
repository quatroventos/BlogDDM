...
function quatroventos_add_google_fonts() {
    wp_enqueue_style( 'wpb-google-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;700&family=Roboto+Slab:wght@300;400;700&display=swap', false );
}
add_action( 'wp_enqueue_scripts', 'quatroventos_add_google_fonts' );

function quatroventos_add_owl_carousel() {
    wp_enqueue_style( 'owl', get_template_directory_uri().'/css/lib/owl.carousel.min.css', false );
    wp_enqueue_script('owl-js', get_template_directory_uri().'/js/lib/owl.carousel.min.js', 'jquery');
}
add_action( 'wp_enqueue_scripts', 'quatroventos_add_owl_carousel' );
